...
export const SAMPLE_BAT_FULL: Sample = {
  id: "bat-ts",
  title: "Полный .bat · ts",
  hint: "Обёртка service.bat + winws, fooling=ts и seqovl",
  source: `@echo off
chcp 65001 > nul
:: 65001 - UTF-8
...
export const SAMPLE_ARGS_BADSEQ: Sample = {
  id: "args-badseq",
  title: "Фрагмент · badseq",
  hint: "Только аргументы winws, без .bat-шапки",
  source: `--filter-udp=443 --hostlist="%LISTS%list-general.txt" --hostlist="%LISTS%list-general-user.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --hostlist-exclude="%LISTS%list-exclude-user.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude-user.txt" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fake-quic="%BIN%quic_initial_www_google_com.bin" --new ^
...
export const SAMPLES = [SAMPLE_BAT_FULL, SAMPLE_ARGS_BADSEQ];
export const DEFAULT_SAMPLE = SAMPLE_ARGS_BADSEQ;
